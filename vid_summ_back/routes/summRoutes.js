...
router.post("/summarize", async (req, res) => {
  try {
    ...
    const result = await model.generateContent([
      "Make a summary of this video tackling the main points and in the first line give me only the title: ",
      {
        fileData: {
          fileUri: youtubeLink,
        },
...
router.delete("/deletesummary/:id", async (req, res) => {
  try {
    ...
    const user = await User.findById(userID)
    user.summaries = user.summaries.filter((summary) => summary._id.toString() !== summaryId)
    await user.save()
...
router.get("/getsummary", async (req, res) => {
  try {
    ...
    const user = await User.findOne(
      { _id: userID, "summaries._id": summaryId },
      { "summaries.$": 1 }
    )
